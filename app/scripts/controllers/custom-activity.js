...
angular.module('gcbCreatorApp').controller('CustomActivityCtrl',['$scope', function ($scope) {
    ...
    $scope.HacerPeticion = function(){ 
        ...
        for (var key in $scope.pregunta){
            $scope.pregunta[key] = $.trim($scope.pregunta[key]);
            if($scope.pregunta[key] == '')
                delete $scope.pregunta[key];
            else{
                //Comprobamos para los HTML
                var aux = $.trim($scope.pregunta[key].replace(/(&nbsp;)|(<[\/]?div>)|(<br[\/]?>)/gi,''));
                if(aux == '')
                    delete $scope.pregunta[key];
            }
        }
